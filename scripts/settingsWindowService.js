...
function settingsWindowService(projectService, deviceService, popupService, $rootScope, $cookies, variableService, $timeout, helpService) {
    ...
    var settingsWindowHandler = {
        ...
        saveDiSettings: function () {
            ...
            localStorage.setItem("reconnect", deviceService.reconnect);
            localStorage.setItem("localAddons", deviceService.localAddons);
        ...
        setCurrentTab: function (tab, group) {
            ...
            var scope = angular.element("#settingsTab .helpIcon").scope();

            if (!scope) return;

            scope.$parent.help = tab.help;
        },
        ...
        trial: {
            password: localStorage.getItem("trialPassword") == "null" ? null : localStorage.getItem("trialPassword"),
            username: localStorage.getItem("trialUsername") == "null" ? null : localStorage.getItem("trialUsername"),
            mode: localStorage.getItem("trialMode") == "true" ? true : false,
            readMore: function () {
                window.open($rootScope.flexguiSettings.enterpriseTrialUrl, "_system");
            },
            save: function () {

                if ($rootScope.flexguiSettings.buildInPluginLogin) {
                    localStorage.setItem("trialPassword", settingsWindowHandler.trial.password);
                    localStorage.setItem("trialUsername", settingsWindowHandler.trial.username);
                }
                localStorage.setItem("trialMode", settingsWindowHandler.trial.mode);
    ...
    $rootScope.settingsTabs.enterprise = { help: helpService.settings.enterprise, children: [], position: 7, source: "views/settings/enterprise.html", title: localization.currentLocal.settings.tabs.enterprise.title, classes: "enterpriseSettingsTab" };

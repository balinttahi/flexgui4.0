...
function shapeCtrl($http, $sce, $scope, $window, $location, $routeParams, $attrs, editorService, popupService, deviceService, scriptManagerService, projectService, variableService, settingsWindowService, $timeout, $rootScope) {
    ...
    $scope.initShapeFidget = function (fidget) {
        if (!fidget) return;

        $scope.currentFidget = fidget;

        function reloadHtml () {
        	//	Get a common shape or a polygon
        	var file = fidget.properties.shape;
        	switch (fidget.properties.shape) {
        	...
        	if (file == "polygon") {
        		...
        		var oWidth = -1000000;
        		var oHeight = -1000000;
        		for (var i = 0; i < polyShapes[fidget.properties.shape].points.length; i++) {
        			oLeft = polyShapes[fidget.properties.shape].points[i].x < oLeft ? polyShapes[fidget.properties.shape].points[i].x : oLeft;
        			oTop = polyShapes[fidget.properties.shape].points[i].y < oTop ? polyShapes[fidget.properties.shape].points[i].y : oTop;
        			oWidth = polyShapes[fidget.properties.shape].points[i].x > oWidth ? polyShapes[fidget.properties.shape].points[i].x : oWidth;
        			oHeight = polyShapes[fidget.properties.shape].points[i].y > oHeight ? polyShapes[fidget.properties.shape].points[i].y : oHeight;
        		}
        		oWidth -= oLeft;
        		oHeight -= oTop;
        		for (var i = 0; i < polyShapes[fidget.properties.shape].points.length; i++) {
        			var newx = fidget.properties.stroke / 2 + (polyShapes[fidget.properties.shape].points[i].x - oLeft) * (fidget.properties.width - fidget.properties.stroke) / oWidth;
        			var newy = fidget.properties.stroke / 2 + (polyShapes[fidget.properties.shape].points[i].y - oTop) * (fidget.properties.height - fidget.properties.stroke) / oHeight;
        			fidget.polyPoints[i] = newx + "," + newy;
        		}
        	}
        	...
        	if (file == "path") {
        		...
        		var pathRest = pathShapes[fidget.properties.shape].path;
        		var scale = pathShapes[fidget.properties.shape].scale;
        		...
        		if (!error) {
					...
					if (scale.x == 0 && scale.y == 0) {
						console.log("scale.x: " + (fidget.properties.width - fidget.properties.stroke) / oWidth);
						console.log("scale.y: " + (fidget.properties.height - fidget.properties.stroke) / oHeight);
					}

					if (scale.x == 0 && scale.y == 0) {
						fidget.path = 'm' + (fidget.properties.stroke / 2 + oLeft) + "," + (fidget.properties.stroke / 2 + oTop);
					}
					else {
						fidget.path = 'm' + (fidget.properties.stroke / 2 + oLeft / scale.x * (fidget.properties.width - fidget.properties.stroke) / oWidth) + "," + (fidget.properties.stroke / 2 + oTop / scale.y * (fidget.properties.height - fidget.properties.stroke) / oHeight);
					}
					for (var i = 0; i < oPath.length; i++) {
						var newx, newy;
						if (scale.x == 0 && scale.y == 0) {
							...
							newx = oPath[i].x / scale.x * (fidget.properties.width - fidget.properties.stroke) / oWidth;
							newy = oPath[i].y / scale.y * (fidget.properties.height - fidget.properties.stroke) / oHeight;
						}
						fidget.path += oPath[i].command + newx + "," + newy;
					}
        		}
        		else
        			console.log("ERROR! Couldn't generate " + fidget.properties.shape + " shape's path");
        	}

        	$scope.shapeHtml = $sce.trustAsResourceUrl($rootScope.addonServerUrl + "addons/extendedFidgets/views/fidgets/shapes/" + file + ".html");
        }

        $scope.$watch(function () { return fidget.properties.shape; }, reloadHtml);
        $scope.$watch(function () { return fidget.properties.width; }, reloadHtml);
        $scope.$watch(function () { return fidget.properties.height; }, reloadHtml);
        $scope.$watch(function () { return fidget.properties.stroke; }, reloadHtml);

        $scope.$watch(function () { return projectService.loaded; }, function () {
            //update reference if project loaded
            fidget = projectService.getFidgetById(fidget.id);
        });

        //calculate the bounding rectangle
        $scope.$watchGroup([function () { return fidget.properties.width; }, function () { return fidget.properties.height; }, function () { return fidget.properties.angle; }], function () {

            //allow angle from 0->359 degree
            while (fidget.properties.angle < 0) {
                fidget.properties.angle += 360;
            }

            //rotate size
            fidget.properties.angle = fidget.properties.angle % 360;
            $scope.size = variableService.fitRect(editorService.getFidgetSize(fidget), fidget.properties.angle * (Math.PI / 180));
            $scope.center = { x: (fidget.properties.width - $scope.size.width) / 2, y: (fidget.properties.height - $scope.size.height) / 2 };
        });
    }
}
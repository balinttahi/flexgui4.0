...
function slamMapCtrl($http, $sce, $scope, $window, $location, $routeParams, $attrs, editorService, popupService, deviceService, scriptManagerService, projectService, variableService, settingsWindowService, $timeout, $rootScope) {
    ...
    var draw = function (pos) {
        ...
        var rob = $scope.robot;
        var mapCanvas = document.getElementById('slamCanvas_' + fidget.id);
        ...
        image.onload = function () {
            ...
            mapCtx.translate(fidget.properties.width / 2 + fidget.properties.hpan, fidget.properties.height / 2 + fidget.properties.vpan); //change origin
            mapCtx.rotate(fidget.properties.angle * Math.PI / 180); // rotate
            mapCtx.translate(-fidget.properties.width / 2, -fidget.properties.height / 2); //change origin back
            mapCtx.drawImage(image, 0, 0, meta.width, meta.height, 0, 0, fidget.properties.width * fidget.properties.zoom, fidget.properties.height * fidget.properties.zoom);

            var fidgetRatio = fidget.properties.width / fidget.properties.height;
            ...
            if (rob.image && rob.left > -1 && rob.top > -1) {
                mapCtx.drawImage(rob.image,
                    ...
                    (rob.left - rob.image.width * (fidget.properties.width / meta.width) / 2) * fidget.properties.zoom, (rob.top - rob.image.height * (fidget.properties.height / meta.height)) * fidget.properties.zoom,
                    fidget.properties.width * fidget.properties.zoom, fidget.properties.height * fidget.properties.zoom);
            }
            ...
            if (dest.image && dest.show) {
                mapCtx.drawImage(dest.image,
                    0, 0,
                    meta.width, meta.height,
                    (dest.left - dest.image.width * (fidget.properties.width / meta.width) / 2) * fidget.properties.zoom, (dest.top - dest.image.height * (fidget.properties.height / meta.height)) * fidget.properties.zoom,
                    fidget.properties.width * fidget.properties.zoom, fidget.properties.height * fidget.properties.zoom);
            }
    ...
    //init fidget
    $scope.initMapFidget = function (fidget) {
        if (!fidget) return;
        ...
        $scope.$watch(function () { return projectService.loaded; }, function () {
            ...
            $timeout(function () {
                ...
                positionTopic = new ROSLIB.Topic({
                    name: fidget.properties.positionTopicName,
            ...
            //resize map
            $scope.$watchGroup([function () { return fidget.properties.width; }, function () { return fidget.properties.height; }], function () {
                var dest = $scope.destination;
                var rob = $scope.robot;
            ...
            $scope.$watch(function () { return fidget.properties.positionTopicName; }, function (nv, ov) {
                if (ov && positionTopic) {
                ...
                if (nv) {
                    ...
                    positionTopic.subscribe(function (message) {
                        ...
                        robot.top = pos.y * fidget.properties.height / meta.height;
                        robot.left = pos.x * fidget.properties.width / meta.width;
            ...
            $scope.$watch(function () { return fidget.properties.destinationTopicName; }, function (nv, ov) {
                ...
                if (nv) {
                    ...
                    destinationTopic = new ROSLIB.Topic({
                        name: fidget.properties.destinationTopicName,
                        messageType: 'geometry_msgs/Vector3',
                        ros: variableService.ros
            ...
            //redraw
            $scope.$watchGroup([
                    function () { return fidget.properties.angle; },
                    function () { return fidget.properties.zoom; },
                    function () { return fidget.properties.hpan; },
                    function () { return fidget.properties.vpan; }],
